...
FirstFitSolver::FirstFitSolver(std::vector<std::shared_ptr<Problem>>& problems,
...
void FirstFitSolver::PrintProblemWeights(
...
std::shared_ptr<SolutionContainer> FirstFitSolver::Solve(std::shared_ptr<Problem> the_problem)
{
...
void FirstFitSolver::StartTimer()
{
	time(&m_start_time);
}

long long FirstFitSolver::GetDuration() const
{
	return difftime(time(0),m_start_time);
}

long long FirstFitSolver::StopTimer() const
{
	return GetDuration();
}
